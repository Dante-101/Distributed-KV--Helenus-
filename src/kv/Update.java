...
public class Update {
	...
	public Update(KeyValue kv, int quorum){
		this.kv = kv;
	...
	public void updateKeyValue(){
		...
		if(self){
			...
			if(!this.isKeyPresent()){
				String warn = "The key " + kv.getKey() + " is not present"; 
				Log.warn(warn);
			}

			if(this.successful){
				String info = "Updated key " + kv.getKey() + " with value " + kv.getValue().get(); 
				Log.keyUpdate(info);
			} else {
		...
		synchronized(lockObj){
			for(int i=0; i<numServersToContact; i++){
				crdTh[i] = new CoordinatorThread(KVServer.CMDUPDATE, 
